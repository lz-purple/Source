...
class RfSwitchApBoxTest(unittest.TestCase):
    ...
    def testGetApsList(self):
        ...
        self.assertEquals(ap_box.ap_box_label, 'ap_box_1')
        self.assertEquals(ap_box.rf_switch_label, 'rf_switch_1')
        aps = ap_box.get_ap_list()
        self.assertEquals(len(aps), 1)
        ap = aps[0]
        self.assertEquals(ap.get_wan_host(), 'chromeos9-ap1')
        self.assertEquals(ap.get_bss(), '1a:2b:3c:4d:5e:6f')
